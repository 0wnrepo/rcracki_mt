			...
			if (!doOldFormat) // read index
			{
				...
				if(fIndex != NULL)
				{
					...
					else
					{
						...
						//int nRead = 0;
						//int nDataRead = 0;
						unsigned int nDataRead = fread(pIndex, 1, nFileLen, fIndex);
						...
						//{
							...
							//if(nRead == 11)
							//{
							//	printf("%s - %u -  %u\n",uint64tostr(pIndex[i].nPrefix).c_str(), pIndex[i].nFirstChain, pIndex[i].nChainCount );
							//}
							//else break;
						//}
						clock_t t2 = clock();
				...
				if (pChain != NULL)
				{
					...
					while (true)	// Chunk read loop
					{
						...
						if (!fVerified)
						{
							...
							uint64 nIndexS;
							//if (doOldFormat)
							//	nIndexS = pChain[nIndexToVerify].nIndexS;
							//else
